...
import TeamForm from "@/components/accessibility-services/AccessibilityForm1";
...
const TeamDetails = () => {
    ...
    const { accessibilityId } = useParams(); // get teamId from useParams

    const getDetails = async () => {
        try {
            const res = await fetch(`/api/accessibility-services/${accessibilityId}`, {
                method: "GET"
            });
    ...
    const renderForm = () => {
        ...
        const FormComponent = formMapping[accessibilityId as TeamId] || TeamForm;
