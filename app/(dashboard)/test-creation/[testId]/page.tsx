...
import TeamForm from "@/components/test-creation/TestForm1";
...
const TeamDetails = () => {
    ...
    const { testId } = useParams(); // get teamId from useParams

    const getDetails = async () => {
        try {
            const res = await fetch(`/api/test-creation/${testId}`, {
                method: "GET"
            });
    ...
    const renderForm = () => {
        ...
        const FormComponent = formMapping[testId as TeamId] || TeamForm;
