    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            ...
            const url = initialData
                ? `/api/video-creation/${initialData._id}`
                : "/api/video-creation";
            ...
            if (res.ok) {
                setLoading(false);
                toast.success(`Video-Creation ${initialData ? "updated" : "created"}`);
                window.location.href = "/video-creation";
                router.push("/video-creation");
            }
        } catch (err) {
            console.log("[video-creation_POST]", err);
        ...
        <div className="p-10">
            {initialData ? (
                <div className="flex items-center justify-between">
                    <p className="text-heading2-bold">Edit Video-Creation</p>
                    {/* <Delete id={initialData._id} item="video-creation" /> */}
                </div>
            ) : (
                <p className="text-heading2-bold">Video-Creation</p>
            )}
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Textarea
                                        placeholder="Service Descriprion"
                                        {...field}
                                        rows={4}
                                        onKeyDown={handleKeyPress}
                                        className="custom-border"
