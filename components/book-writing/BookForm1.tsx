...
const formSchema = z.object({
    ...
    field11: z.string().max(500).optional(),
    field12: z.string().max(500).optional(),
    field13: z.string().max(500).optional(),
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            ...
            const url = initialData
                ? `/api/book-writing/${initialData._id}`
                : "/api/book-writing";
            ...
            if (res.ok) {
                setLoading(false);
                toast.success(`Book-Writing ${initialData ? "updated" : "created"}`);
                window.location.href = "/book-writing";
                router.push("/book-writing");
            }
        } catch (err) {
            console.log("[book-writing_POST]", err);
        ...
        <div className="p-10">
            {initialData ? (
                <div className="flex items-center justify-between">
                    <p className="text-heading2-bold">Edit Book-Writing</p>
                    {/* <Delete id={initialData._id} item="book-writing" /> */}
                </div>
            ) : (
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 font-semibold">
                    ...
                    <div className="flex gap-10">
                        ...
                        <Button
                            type="button"
                            onClick={() => router.push("/book-writing")}
